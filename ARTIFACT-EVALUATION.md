...
At least 40 GB of storage and 20GB of RAM. NVIDIA GPU is recommended but not necessary to reproduce the results. The setup process described in the README is tested on Ubuntu 20.04 with x86-64 architecture with Python 3.8. The estimated time to perform data obfuscation on all 3 datasets will take 1-2 days on a single NVIDIA RTX 2080 Ti GPU. However, the reviewer can choose to interrupt the data obfuscation process and only evaluate on a reasonable amount of batches of the dataset to reduce the computational cost as discussed in README.
...
We recommend using Ubuntu 20.04 and Python 3.8 to evaluate our artifact. We provide all datasets and pre-trained models on Google Drive (https://drive.google.com/file/d/1Hwjhe6v0ZfoSshPA7CIXjwzST9CeSkRD/view?usp=sharing), as described in README. 

For the ease of evaluation, we recommend using a x86-64 machine to install our Docker image (this does not support GPU acceleration). Note that our Docker image already contains all datasets and pre-trained models.

### Estimated Time and Storage Consumption
The storage consumption for using our docker image will take ~35GB. 

The estimated time for evaluating all 3 datasets would take around 2 days with GPU acceleration. Evaluating all 3 datasets on CPU only would take multiple days depending on the computational power of CPU.
...
We also provide pre-built Docker image on Docker Hub with all datasets and models included, which can be obtained by running: `docker pull neilyxin/privdiffuser`
...
#### Using Docker Image:
We recommend using our Docker image as it has all necessary dependencies installed and contains all source code, datasets, and pre-trained models. 
...
We recommend install [miniconda](https://www.anaconda.com/docs/getting-started/miniconda/install) and create a conda environment with Python 3.8: 
```bash
conda create -n "myenv" python=3.8
```

We provide a list of dependencies with their versions in `requirements.txt`. Reviewers need to install the listed dependencies using `conda install` or `pip install`. The reviewer might refer to the official website of TensorFlow and PyTorch to install GPU-accelerated versions.
...
The result on MobiAct dataset is presented in Section 6.1, Table 1 and Figure 4.
...
The result on MotionSense dataset is presented in Section 6.2, Table 2 and Figure 5.
...
The result on WiFi-HAR dataset is presented in Section 6.3, Table 3 and Figure 6.
...
Change `self.private='gender'` in the `Args` class into `self.private='weight'`, then re-start the kernel and run the Jupyter notebook will reproduce the results for weight obfuscation.

The results should align with Main Result 1. 

#### Experiment 2: Data Obfuscation on MotionSense Dataset
Change `self.dataset='mobi'` in the `Args` class into `self.dataset='motion'`, set `self.private='gender'`, then re-start the kernel and run the Jupyter notebook will reproduce the results for gender obfuscation on MotionSense.

The results should align with Main Result 2. 
#### Experiment 3: Data Obfuscation on WiFi-HAR Dataset 
Set `self.dataset='wifi'` and `self.private='weight'` in the `Args` class, then re-start the kernel and run the Jupyter notebook will reproduce the results for weight obfuscation on WiFi-HAR.

The results should align with Main Result 3. 
## Limitations (Only for Functional and Reproduced badges)
This source code provides main performance evaluation for PrivDiffuser on 3 public datasets, which is discussed in Section 6.1, 6.2, and 6.3. These results are critical as they demonstrate the data obfuscated by PrivDiffuser can reduce intrusive inference accuracy on the private attribute with minimum sacrifice on data utility, outperforming state-of-the-art GAN-based obfuscation models.
...
The configurations of PrivDiffuser are set in the `Args` and `DiffusionArgs` class, where users can easily adjust. PrivDiffuser already supports 3 datasetes and the script for loading the datasets are provided in `dataset_loader.py`. Users can refer to this script to support more datasets.

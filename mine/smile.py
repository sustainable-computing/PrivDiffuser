...
class EMALoss(torch.autograd.Function):
    ...
    def forward(ctx, input, running_ema):
        ...
        # print(input.exp().detach().cpu().numpy())
        input_log_sum_exp = torch.clamp(input.exp(), min=torch.exp(-torch.tensor(tau).to(input.device)), max=torch.exp(torch.tensor(tau).to(input.device))).mean().log()
